...
class BarcodeScannerApp extends AppServer {
  ...
  protected async onSession(session: AppSession, sessionId: string, userId: string) {
    ...
    await session.audio.speak('Product scanner ready. Say scan or press the button. Rotate the product slowly.');
    session.events.onTranscription(async t => {
      ...
      if(data.phase==='streaming'){ if(isNoExpiryPhrase(text)){data.extraction.noExpiryConfirmed=true;await session.audio.speak('Got it, no expiration. Saving.');await this.saveAndFinish(session,sessionId,data);} else { const dd=extractDateFromSpeech(text); if(dd){data.extraction.expiryDate=dd;data.announced.expiry=true;await session.audio.speak(`${buildExpiryLine(dd)}. Saving.`);await this.saveAndFinish(session,sessionId,data);} else if(text.includes('save')||text.includes('done'))await this.saveAndFinish(session,sessionId,data); else if(text.includes('cancel')||text.includes('stop'))await this.stopScan(session,sessionId,data,true); } }
    });
    session.events.onButtonPress(async btn => {
      if(btn.pressType==='short'){ if(data.phase==='idle')await this.startScan(session,sessionId,data); else if(data.phase==='streaming')await this.saveAndFinish(session,sessionId,data); }
      else if(btn.pressType==='long'){ if(data.phase==='streaming')await this.stopScan(session,sessionId,data,true); else await this.readList(session,data); }
    });
  ...
  private async stopScan(s: AppSession, _: string, data: SessionData, announce: boolean) {
    ...
    await db.query('DELETE FROM active_streams WHERE user_id=$1',[data.userId]);data.phase='idle';data.hlsUrl=null;if(announce)await s.audio.speak('Scan cancelled.');
  }
  ...
  private scheduleNext(s: AppSession,sid: string,data: SessionData){
    ...
    data.loopTimer=setTimeout(async()=>{if(!data.loopActive||data.phase!=='streaming')return;try{await this.analyzeFrame(s,data);}catch(e){}this.scheduleNext(s,sid,data);},delay);
  }
  private async analyzeFrame(s: AppSession, data: SessionData) {
    ...
    const result=parseJSON<{product_name:string|null;manufacturer:string|null;barcode:string|null;expiry_date:string|null}>(text);
    if(!result)return;
  ...
  protected async onStop(sessionId: string,_uid: string,reason: string){const data=this.sessions.get(sessionId);if(data){this.stopLoop(data);this.sessions.delete(sessionId);}console.log(`Session ${sessionId} ended: ${reason}`);}
}
